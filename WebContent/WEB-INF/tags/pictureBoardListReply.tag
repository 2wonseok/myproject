	...
	<ul>
		<li style="border-top:none;">
			...
			<p>
				...
				<span>
					<c:if test="${reply.memberid == authUser.id }">
						<a href="${root }/LwsProject/pictureReplyRemove?no=${reply.id }&memberid=${reply.memberid}&postNo=${postNo}&pageNo=${pageNo}" 
			...
			<div class="lws" hidden >
				<form action="pictureCommentAdd" method="post">
					...
					<p>
						...
						<span>
							<c:if test="${comment.memberid == authUser.id }">
								<a href="${root }/LwsProject/pictureCommentReplyRemove?replyid=${comment.commentReplyId }&memberid=${comment.memberid}&postNo=${postNo}&pageNo=${pageNo}" 
									onclick="return confirm('삭제하시겠습니까?')" style="color:red;">삭제</a>
							</c:if>
						</span>
					</p>
