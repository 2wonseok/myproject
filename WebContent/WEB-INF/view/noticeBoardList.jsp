...
<div class="boardsearchbox">
	...
	<table class="table table-hover">
			...
			<c:otherwise>
				<c:forEach items="${noticeBoard.content }" var="boardData">
					<tr>
						<td>
							<c:choose>
						...
						<td>
							<c:choose>
								<c:when test="${boardData.notice == 0 }">
									...
									<a style="color: #ed1b23; font-size: 16px; line-height: 20px;"
										href="noticeBoardView?pageNo=${noticeBoard.currentPage }&post_no=${boardData.post_no }">
										${boardData.title } 
								...
								<c:otherwise>
									<a href="noticeBoardView?pageNo=${noticeBoard.currentPage }&post_no=${boardData.post_no }" style="color: #000;">
										${boardData.title } 
										<c:if test="${boardData.replyCnt > 0 }">
											[${boardData.replyCnt }]
										</c:if>
	...
	<c:if test="${authUser.manager == 0 }">
		<input type="button" onclick="location.href='noticeBoardWrite'" value="글쓰기" id="btn_write" />
	</c:if>
	<div class="pagenation-container d-flex justify-content-center">
		<c:if test="${searchResult.hasArticles() }">
			<nav aria-label="Page navigation example"> 
			  <ul class="pagination mt-3">
				    ...
				    <li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${searchResult.startPage - 5 }">Previous</a></li>
					</c:if>
					<c:forEach var="pNo" begin="${searchResult.startPage }" end="${searchResult.endPage }">
			    	<li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${pNo }&searchKeyword=${searchKeyword}&searchField=${searchField}">${pNo }</a></li>
					</c:forEach>
					<c:if test="${searchResult.endPage < searchResult.totalPages }">
			    	<li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${freeBoard.startPage + 5 }">Next</a></li>
					</c:if>
			  </ul>
			</nav>
		</c:if>
	...
	<div class="pagenation-container d-flex justify-content-center">
		<c:if test="${noticeBoard.hasArticles() }">
			<nav aria-label="Page navigation example"> 
			  <ul class="pagination mt-3">
					<c:if test="${noticeBoard.startPage > 5 }">
				    <li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${noticeBoard.startPage - 5 }">Previous</a></li>
					</c:if>
					<c:forEach var="pNo" begin="${noticeBoard.startPage }" end="${noticeBoard.endPage }">
			    	<li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${pNo }">${pNo }</a></li>
					</c:forEach>
					<c:if test="${noticeBoard.endPage < noticeBoard.totalPages }">
			    	<li class="page-item"><a class="page-link" href="noticeBoard?pageNo=${noticeBoard.startPage + 5 }">Next</a></li>
					</c:if>
			  </ul>
			</nav>
		</c:if>
