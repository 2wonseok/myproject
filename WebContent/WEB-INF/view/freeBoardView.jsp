...
<script>
	function removeCheck() {
		if(confirm("정말 삭제하시겠습니까?")) {
			alert('삭제가 완료되었습니다.');
			location.href = "freeBoardRemove?pageNo=${param.pageNo }&post_no=${post.post_no }";
		 } else {
		 	return;
		 }
	}
...
<section id="container">
	<div class="webzineTypeView">
		...
		<a href="freeBoardView?pageNo=${currentPage }&post_no=${post.post_no }&oper=like" onclick="overClick()" id="btn_like">
		...
		<c:set var="pageNo" value="${empty param.pageNo ? '1' : param.pageNo }"/>
			<input type="button" value="목록" id="btn_list" onclick="location.href='freeBoard?pageNo=${pageNo }&searchKeyword=${param.searchKeyword }&searchField=${param.searchField}'"/>
		<c:if test="${authUser.id == post.memberid }">
			<input type="button" value="수정" id="btn_modify" onclick="location.href='freeBoardmodify?pageNo=${pageNo}&post_no=${post.post_no }'"/>
		</c:if>
		
		<u:replyForm postNo="${post.post_no }" pageNo="${pageNo }" replyId="${reply.id }"/>
		<u:listReply postNo="${post.post_no }" pageNo="${pageNo }"/>
</section>
</body>
</html>