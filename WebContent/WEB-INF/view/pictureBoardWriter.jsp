...
<section id="container">
	<h3>게시글 작성</h3>
	<form action="${root }/LwsProject/pictureBoardWrite" method="post" enctype="multipart/form-data">
	<input type="hidden" name="id" value="${authUser.id }" />
	<input type="hidden" name="name" value="${authUser.name }" />
		<div class="form-group">
	    <label for="input1-title">제목</label>
	    <input type="text" class="form-control" value="${param.title }" name="title" id="input1-title" />
	  ...
	  <div class="form-group">
	    ...
	    <c:if test="${errors.nofile }">
		    <small class="form-text text-primary">사진을 첨부해주세요.</small>
	    </c:if>
	  </div>
